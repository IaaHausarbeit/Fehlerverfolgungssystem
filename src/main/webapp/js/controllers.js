...
controllers.controller('regController', ['$scope', 'Developer', 'developerService', function ($scope, Developer,
                                                                                              developerService) {
    var messages = {
        errors: {
    ...
    this.saveDeveloper = function (regForm) {
        var dev = new Developer(document.getElementById("nameId").value, document.getElementById("nicknameId").value,
            document.getElementById("eMailId").value, document.getElementById("passwordId").value);
...
// formController
controllers.controller('formController', ['$scope', 'Ticket', 'ticketService', 'Commentary',
    function ($scope, Ticket, ticketService, Commentary, $http) {
    var messages = {
        errors: {
            required: 'Pflichtfeld!'
        }
    ...
    $scope.formModel = {
        ...
        formTicket: new Ticket($scope.model.selectedTicket.id, $scope.model.selectedTicket.titel,
            $scope.model.selectedTicket.status,
            $scope.model.selectedTicket.description, $scope.model.selectedTicket.creator,
            $scope.model.selectedTicket.currentWorker,
            $scope.model.selectedTicket.createDateTimestamp, $scope.model.selectedTicket.changeDateTimestamp,
            $scope.model.selectedTicket.commentaryList
        )
    };
    ...
    this.checkBearbeitung = function () {
        if ($scope.model.selectedTicket.status == "ANGELEGT" ||
            $scope.model.selectedTicket.status == "WIEDEREROEFFNET") {
            return true;
        }
    };
    ...
    this.saveTicket = function (editForm, status) {
        ...
        if (editForm.$valid && selected && edited) {
            ...
            if (status == 0) {
                selected.creator = loginName;
            }
            //wenn das Ticket in den Status "in Bearbeitung" überführt wird, ist der Bearbeiter = LoginName und sonst
            // null
            ...
            if (document.getElementById("commentaryID").value != "") {
                var commentary = new Commentary(edited.id, document.getElementById("commentaryID").value, loginName,
                    new Date());
